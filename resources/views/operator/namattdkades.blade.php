<x-layout>
    ...
    <x-slot:tabs>Nama dan Tanda Tangan Kades</x-slot>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addnamattdkades" action="{{ route('addnamattdkades') }}" method="POST"
            ...
            @foreach ($ttd as $item)
                ...
                <td>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#editnamattd{{ $item->id }}">
                        Edit
                    </button>
                </td>

                <div class="modal fade" id="editnamattd{{ $item->id }}" tabindex="-1"
                    aria-labelledby="editnamattdLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                <form id="edit_namattdkades"
                                    action="{{ route('edit_namattdkades', ['id' => $item->id]) }}" method="POST"
                                    enctype="multipart/form-data">
                                    @csrf
                                    <div>
                                        <label for="nama_kades" class="form-label">Nama Kades</label>
                                        <input type="text" class="form-control" id="nama_kades" name="nama_kades"
                                            value="{{ old('nama_kades', $item->nama_kades) }}">
                                        @error('nama_kades')
                                            <div class="alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="file" class="form-control" id="file_baru" name="file_baru"
                                            required>
                                        @error('file_baru')
                                            <div class="alert alert-danger">{{ $message }}</div>
                                        @enderror
