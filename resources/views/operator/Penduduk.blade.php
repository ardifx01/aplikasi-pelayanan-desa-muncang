...
<x-layout>
    ...
    <x-slot:tabs>Penduduk</x-slot>
    <div class="d-flex flex-row bd-highlight">
        <div class="p-1 bd-highlight">
            <form method="GET" action="{{ route('showPenduduk') }}" class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Cari nama atau NIK..." name="cariPenduduk"
                    id="searchInput" value="{{ request('cariPenduduk') }}">
        ...
        <div class="p-1 bd-highlight">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#importExcel">Import
                Excel</button>
        </div>
    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="importPenduduk" action="{{ Route('importPenduduk') }}" method="POST"
                ...
                <tbody id="pendudukTable">
                    @forelse ($datapenduduk as $item)
                        <tr>
                            ...
                            <td>{{ $item->umur_calculated }}</td>
                            <td>{{ $item->kartu_sehat }}</td>
                            <td>{{ $item->agama }}</td>
                            ...
                            <td>
                                <div class="d-flex justify-content-start">
                                    <button type="button" class="btn btn-primary btn-sm me-2" data-bs-toggle="modal"
                                        data-bs-target="#penduduk{{ $item->NIK }}">Edit</button>
                                    <a href="/deletependuduk/{{ $item->NIK }}"
                                        class="btn btn-danger btn-sm hapuspenduduk" data-nik="{{ $item->NIK }}"
                                        data-nama="{{ $item->nama }}">Hapus</a>
                        ...
                        <div class="modal fade" id="penduduk{{ $item->NIK }}" tabindex="-1"
                            data-bs-keyboard="false" aria-labelledby="pendudukLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="pendudukLabel">Edit Penduduk {{ $item->NIK }}
                                        </h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form id="editpenduduk"
                                            action="{{ route('editpenduduk', ['NIK' => $item->NIK]) }}" method="POST">
                                            @csrf
                                            <fieldset>
                                                <div>
                                                    <label for="NIK" class="form-label">NIK</label>
                                                    <input type="text" class="form-control" id="NIK"
                                                        name="NIK" value="{{ $item->NIK }}" disabled>
                                                </div>
                                                <div class="mb-1">
                                                    <label for="kk" class="form-label">KK</label>
                                                    <input type="text" class="form-control" id="kk"
                                                        name="kk" value="{{ $item->kk }}">
                                                </div>
                                                <div class="mb-1">
                                                    <label for="kartu_sehat" class="form-label">Nomor Kartu
                                                        Sehat</label>
                                                    <input type="text" class="form-control" id="kartu_sehat"
                                                        name="kartu_sehat" value="{{ $item->kartu_sehat }}">
                                                </div>
                                                <div class="mb-1">
                                                ...
                                                <div class="mt-2 mb-1">
                                                    ...
                                                    <select class="form-select" id="kewarganegaraan"
                                                        ...
                                                        <option value="WNI"
                                                            {{ $item->kewarganegaraan == 'WNI' ? 'selected' : '' }}>WNI
                                                        </option>
                                                        <option value="WNA"
                                                            {{ $item->kewarganegaraan == 'WNA' ? 'selected' : '' }}>WNA
                                                        </option>
                                                    </select>
                                                </div>
                                            </fieldset>
