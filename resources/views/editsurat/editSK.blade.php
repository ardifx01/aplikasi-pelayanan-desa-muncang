            ...
            <div>
                <div class="col-sm-10">
                    <input type="text" name="nomor_surat" id="nomor_surat" class="form-control"
                        value="{{ $surat->nomor_surat }}" disabled></input>
                </div>
            </div>
        </fieldset>
        <fieldset class="mt-2">
            ...
            <div class="mt-2 mb-3 row">
                ...
                <div class="col-sm-10">
                    <select id="searchPemohon" class="form-select">
                        @if ($surat)
                            <option value="{{ $surat->nik_pemohon }}" selected>{{ $surat->nama_pemohon }} |
                                {{ $surat->nik_pemohon }}</option>
                        @endif
                    </select>
            ...
            <div class="card col-sm-10" style="display: none;" id="pemohonDetails">
                <div class="card-body">
                    ...
                    <p class="card-text">Tempat Tanggal Lahir: <span id="tempat_lahir"></span>, <span
                            id="tanggal_lahir"></span></p>
                </div>
            </div>
        </fieldset>
        <fieldset>
            ...
            <div>
                <div class="col-sm-10">
                    ...
                    <textarea name="alamat_lama" id="alamat_lama" cols="5" rows="5" class="form-control">{{ $sk->alamat_lama ?? '' }}</textarea>
                </div>
            </div>
            <div>
                <div class="col-sm-10">
                    <label for="alamat_baru" class="form-label">Alamat Baru</label>
                    <textarea name="alamat_baru" id="alamat_baru" cols="5" rows="5" class="form-control">{{ $sk->alamat_baru ?? '' }}</textarea>
                </div>
            </div>
            <div>
                <div class="col-sm-10">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#searchPemohon').on('select2:select', function(e) {
                ...
                $('#jenis_kelamin').text(data.additionalData.jenis_kelamin === 'L' ? 'Laki-laki' :
                    'Perempuan');
                $('#tempat_lahir').text(data.additionalData.tempat_lahir);
                var formattedTanggalLahir = new Date(data.additionalData.tanggal_lahir).toLocaleDateString(
                    'id-ID');
